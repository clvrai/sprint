...
from sprint.models.enc_lang import EncoderLang
import numpy as np

from sprint.models.enc_vl import EncoderVL
import sprint.utils.model_utils as model_utils
import sprint.utils.data_utils as data_util
from sprint.models.base_model import FeatureFlat, ObjectClassifier, ETOfflineRLBaseModel
from sprint.alfred.gen import constants
...
class SPRINTETIQLModel(nn.Module):
    def __init__(self, args, pad=0, seg=1):
        ...
        self.vocab_word["action_low"] = torch.load(
            f"{os.environ['SPRINT']}/sprint/models/low_level_actions.vocab"
        )[
            "action_low"
        ]  # our custom vocab
        ...
        self.pad = pad
        self.seg = seg
        self.use_llm_labels = args.use_llm_labels
        self.chain_multi_trajectory = args.chain_multi_trajectory
    ...
    def train_offline_from_batch(
        ...
        rewards,
        terminals,
        state_goal=None,  # for Actionable Models Implementation
        state_text_instructions_list=None,  # for cross-traj chaining
        state_text_summaries=None,  # for cross-traj chaining
        state_text_summaries_list=None,  # for cross-traj chaining
        lengths_state_text_summaries=None,  # for cross-traj chaining
        eval=False,
    ):
        if self.chain_multi_trajectory:
            accum_factor = 0.5
        else:
            accum_factor = 1.0
        ret_dicts = []
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            # in trajectory
            if self.use_llm_labels and state_text_summaries is not None:
                in_traj_lang = state_text_summaries
                in_traj_lengths_lang = lengths_state_text_summaries
            else:
                in_traj_lang = lang
                in_traj_lengths_lang = lengths_lang

            in_traj_reward = rewards  # .unsqueeze(-1)
            in_traj_terminal = terminals  # .unsqueeze(-1)
            ret_dicts.append(
            ...
            # cross-trajectory multi-step chaining (Eq 3)
            if self.chain_multi_trajectory and not eval:
                with torch.no_grad():
                    skill_agg_instructions_list = (
                        state_text_summaries_list
                        if self.use_llm_labels and state_text_summaries_list is not None
                        else state_text_instructions_list
                    )
                    shifted_state_text_instructions = (
                        skill_agg_instructions_list[1:]
                        + skill_agg_instructions_list[:1]
                    )

                    combined_sentences = []
                    # combine sentences with an "and"
                    and_tok = data_util.tokenize_sentences(
                        ["and"], self.critics.vocab_word
                    )[0][0]
                    for i in range(len(skill_agg_instructions_list)):
                        combined_sentences.append(
                            torch.cat(
                                (
                                    skill_agg_instructions_list[i][:-1],
                                    and_tok,
                                    shifted_state_text_instructions[i],
                                )
                            )
                        )

                    # join text instructions after stripping padding
                    skill_aggregation_lengths_lang = torch.tensor(
                        [len(s) for s in combined_sentences]
                    )
                    skill_aggregation_lang = (
                        pad_sequence(
                            combined_sentences,
                            batch_first=True,
                            padding_value=0,
                    ...
                    # Generate update for first trajectory aggregation
                    prev_training = self.training
                    self.eval()
                    shifted_lang = torch.roll(in_traj_lang, -1, 0)
                    shifted_lang_lengths = torch.roll(in_traj_lengths_lang, -1, 0)
                    skill_aggregation_reward = self.critics.get_sequential_v(
                        vocab=self.critics.vocab_word["word"],
                        lang=shifted_lang,
                        lengths_lang=shifted_lang_lengths,
                        frames=frames,
                        lengths_frames=lengths_frames,
                        length_frames_max=lengths_frames.max().item(),
                        action=action,
                    ...
                    # skill_aggregation_reward = torch.cat((skill_aggregation_reward, torch.zeros_like(skill_aggregation_reward[:, :1])), dim=1)
                    skill_aggregation_reward = torch.clamp(
                        skill_aggregation_reward * terminals,
                        min=0,
                        max=1,
                    )

                    # Generate update for second trajectory aggregation. We rolled -1 before, so we just modify aggregation_filter with + 1. E.g. if
                    # we grabbed traj0 before, it got aligned with traj1 now, so we want to grab traj1 here.
                    # same summary, but it's shifted (e.g. summarize traj0->traj1 will be index 0 but should be index1 in the next traj so traj0->traj1 lines up with traj1)
                    skill_aggregation_lang_2 = torch.roll(skill_aggregation_lang, 1, 0)
                    skill_aggregation_lengths_lang_2 = torch.roll(
                        skill_aggregation_lengths_lang, 1, 0
                    )
                    skill_aggregation_reward_2 = terminals
                ...
                ret_dicts.append(
                    self.accum_gradient_from_batch(
                        ...
                        obj_id,
                        lengths_frames,
                        skill_aggregation_lengths_lang,
                        interact_mask.view(-1),
                        skill_aggregation_reward,
                        terminals,
                        accum_factor / 2,
                        eval,
                    )
                ...
                ret_dicts.append(
                    self.accum_gradient_from_batch(
                        frames,
                        skill_aggregation_lang_2,
                        action,
                        obj_id,
                        lengths_frames,
                        skill_aggregation_lengths_lang_2,
                        interact_mask.view(-1),
                        skill_aggregation_reward_2,
                        terminals,
                        accum_factor / 2,
                        eval,
                    )
