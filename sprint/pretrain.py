...
from sprint.dataloaders.sprint_dataloader import SPRINTDataset, collate_func
...
def train_epoch(agent_model, dataloader, epoch, device, save_dir, experiment_name):
    ...
    with tqdm(dataloader, unit="batch") as tepoch:
        for batch_i, data_dict in enumerate(tepoch):
            ...
            rewards = send_to_device_if_not_none(data_dict, "reward", device)
            terminals = send_to_device_if_not_none(data_dict, "terminal", device)
            summary_lang = send_to_device_if_not_none(
                data_dict, "summary_token", device
            ).int()
            ...
            loss_info = agent_model.train_offline_from_batch(
                ...
                terminals,
                state_goal=goal_frames,
                state_text_instructions_list=lang_list,
                state_text_summaries=summary_lang,
...
def main(config):
    ...
    dataset = SPRINTDataset(
        config.data_dir,
        "train",
        sample_primitive_skill=config.sample_primitive_skill,
        max_skill_length=config.max_skill_length,
    )
    valid_seen_dataset = SPRINTDataset(
    ...
    valid_unseen_dataset = SPRINTDataset(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        const=True,
        nargs="?",
        help="whether to use LLM labels for training",
    )
    parser.add_argument(
        "--chain_multi_trajectory",
        type=str2bool,
        default=True,
        const=True,
        nargs="?",
        help="whether to use multi-trajectory cross-trajectory chaining with naive sentence concatenation relabeling (Eq. 5 in paper)",
    )
    parser.add_argument(
